...
- Run `bin/fm-lint.sh` before treating a script change as done; it is the single owner of the lint definition (file set, config, pinned shellcheck version) that the no-mistakes pre-push gate invokes, and it refuses to run under any other shellcheck version.
