...
state/               runtime records and signals; gitignored
  ...
  .<id>.secondmate-respawn-retry  bounded startup handoff left when secondmate liveness meets the exact forced-teardown task-set lock; bin/fm-bootstrap.sh owns its format, retry ceiling, and live-respawn clearing, while bin/fm-teardown.sh removes it on retirement
