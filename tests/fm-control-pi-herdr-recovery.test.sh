...
cat > "$HOME_DIR/data/pirecovery/brief.md" <<'EOF'
# Task
## Captain's intent
Recover the exited Pi probe.

## Firstmate spec
Replace only the stopped Pi process in its recorded pane and local copy.
EOF
...
composer=$(FM_STALE_PI_STATE=1 PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" bash -c '. "$0/bin/fm-backend.sh"; fm_backend_composer_state herdr "$1"' "$ROOT" "$SESSION:$PANE")
[ "$composer" = unknown ] || fail "a bare Pi shell must refuse text injection, got $composer"
