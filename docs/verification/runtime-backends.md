...
- Strict typecheck: on 2026-08-28, with the repository's own npm install pinning `@earendil-works/pi-coding-agent` 0.84.3 and TypeScript 5.9.3, `tests/fm-pi-primary-types.test.sh` printed `ok - tracked Pi extensions pass strict no-emit typecheck against Pi 0.84.3` after the Calm terminal-input handler's return type was corrected to satisfy Pi 0.84.3's `TerminalInputHandler` contract.
...
The extension executes inside the signed CLI's own runtime, so a CLI upgrade can drift ahead of the pinned npm surface; refresh this record after every Pi upgrade by re-running the live guard, picker regression, and strict typecheck above (the test now defaults to the matching npm install the repository pins) and by watching the branch's own fallback line - every branch failure degrades to the pre-branch wake-to-main path by construction, which `tests/fm-pi-branch-extension.test.sh` holds with a broken generator and the live guard holds with the real SDK.
