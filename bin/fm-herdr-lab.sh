...
# Every Herdr call made here carries --session <session> before its subcommand.
...
fm_herdr_lab_cli() { # <session> <herdr arguments...>
  ...
  for arg in "$@"; do
    case "$arg" in
      --session|--session=*)
        fm_herdr_lab_error "run forbids caller-supplied --session; the helper supplies the lab session before the subcommand"
        return 1
        ;;
    esac
  done
